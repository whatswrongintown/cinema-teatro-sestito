...
df %>% filter(anno_proiezione >= 1948 & anno_proiezione <= 1957) %>%
          filter(!is.na(rated)) %>%
          mutate(rating_livello = case_when(
                    rated %in% c("15", "K-16", "TV-14") ~ "Vietato ai minori",
                    rated %in% c("13", "PG", "S") ~ "Parzialmente vietato",
                    rated %in% c("12", "6") ~ "Accesso limitato",
                    rated %in% c("Approved", "Passed", "G") ~ "Accesso libero",
                    TRUE ~ "Altro"
          )) %>%
          mutate(rated = factor(rated, levels = c("15", "K-16", "TV-14", "13", "PG", "S", "12", "6", "Approved", "Passed", "G"))) %>%
