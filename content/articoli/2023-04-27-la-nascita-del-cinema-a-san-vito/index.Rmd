...
df %>% filter(anno_proiezione >= 1948 & anno_proiezione <= 1957) %>%
          count(anno_proiezione) %>%
          ggplot(aes(x = anno_proiezione, y = n)) +
          geom_col(fill = "#CBF8DF") +
          scale_x_continuous(breaks = 1948:1957) +
          labs(
                    title = "Le prime stagioni del Cinema (1948–1957)",
                    subtitle = "Numero di film proposti per anno",
                    x = "Anni",
                    y = "Numero di film"
          )
```
...
df %>% filter(anno_proiezione >= 1948 & anno_proiezione <= 1957) %>%
          filter(!is.na(rated)) %>%
          mutate(rated = case_when(
                    rated == "18" ~ "18+",
                    rated %in% c("15", "14", "K-16", "TV-14", "13") ~ "16+",
                    rated %in% c("PG", "S", "12", "6") ~ "PG",
                    rated %in% c("Approved", "Passed", "G") ~ "Per tutti",
                    TRUE ~ "Altro"
          )) %>%
          mutate(rated = factor(rated, levels = c("18+", "16+", "PG", "Per tutti"))) %>%
          count(anno_proiezione, rated) %>%
          ggplot(aes(x = factor(anno_proiezione), y = n, fill = rated)) +
          geom_bar(stat = "identity") +
          scale_fill_viridis_d(option = "G") +
          labs(
                    title = "Distribuzione dei film per rating (1948–1957)",
                    subtitle = 'Classificazioni sopra "PG" indicano film inadatti alla gioventù e alla morale pubblica',
                    x = "Anni",
                    y = "Numero di film",
                    fill = "",
          ) +
